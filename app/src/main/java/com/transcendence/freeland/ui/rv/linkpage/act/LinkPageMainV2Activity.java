...
import com.transcendence.freeland.ui.rv.linkpage.bean.LinkBean;
import com.transcendence.freeland.ui.rv.linkpage.ui.BaseRecyclerHolder;
import com.transcendence.freeland.ui.rv.linkpage.ui.BaseRecyclerViewAdater;

import java.util.ArrayList;
import java.util.List;

public class LinkPageMainV2Activity extends AppAc {
    ...
    private void initData() {
        ...
        LogUtils.i( linkBean.itemS.size()+"initData: "+linkBean.itemLS.size());
    }

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ui_rv_linkpage_main_v2);
        initData();
        initViews();
        initListener();
    }
    ...
    private void initViews() {
        ...
        lAdapter = new LAdapter(this,R.layout.activity_ui_rv_linkpage_v2_item_left,linkBean.itemLS);
        lAdapter.bindToRecyclerView(rvL);
        rvL.setAdapter(lAdapter);
        rAdapter = new RAdapter(this,R.layout.activity_ui_rv_linkpage_v2_item_right,linkBean.itemS);
    ...
    class LAdapter extends BaseRecyclerViewAdater<LinkBean.ItemL> {
        ...
        public void convert(BaseRecyclerHolder holder, final int position) {
            TextView tv = ((TextView)holder.getView(R.id.tv_left));
            tv.setText(getmData().get(position).getTitle());
            if (checked == position){
                tv.setTextColor(ContextCompat.getColor(context,R.color.colorGitHubGreen));
                tv.setBackgroundResource(R.color.white);
            }else {
                tv.setTextColor(ContextCompat.getColor(context,R.color.text_66));
        ...
        public void convert(BaseRecyclerHolder holder, final int position) {
            ...
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                 ...
                 public void onClick(View view) {
                     Toast.makeText(LinkPageMainV2Activity.this,getmData().get(position).getTitle()+getmData().get(position).getName(),Toast.LENGTH_SHORT).show();
                 }
             });
        }
    }
