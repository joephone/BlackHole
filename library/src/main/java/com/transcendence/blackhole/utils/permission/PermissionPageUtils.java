package com.transcendence.blackhole.utils.permission;
...
/**
 * Created by Joephone on 2019/4/17 18:59
 * E-Mail Address：joephonechen@gmail.com
 */

public class PermissionPageUtils {
    private final String TAG = "xinyu";
...
//        Logs.logE("jumpPermissionPage --- name : " + name);
        switch (name) {
            case "HUAWEI":
                goHuaWeiMainager();
                break;
...
//        Logs.logI("goMiaoMiMainager --- rom : "+rom);
    ...
    private void goIntentSetting() {
        ...
        Uri uri = Uri.fromParts("com.xyyl.doctor.xinyuyiliao", mContext.getPackageName(), null);
        intent.setData(uri);
        try {
            mContext.startActivity(intent);
        } catch (Exception e) {
...
//        Logs.logI("resolveinfoList" + resolveinfoList.size());
        for (int i = 0; i < resolveinfoList.size(); i++) {
//            Logs.logI( resolveinfoList.get(i).activityInfo.packageName + resolveinfoList.get(i).activityInfo.name);
        }
