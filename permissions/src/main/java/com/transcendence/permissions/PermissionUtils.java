...
final class PermissionUtils {
    ...
    static boolean isGrantedStoragePermission(Context context) {
        ...
        return Permissions.isGranted(context, PermissionPool.GROUP.STORAGE);
    }
    ...
    static boolean isSpecialPermission(String permission) {
        return PermissionPool.MANAGE_EXTERNAL_STORAGE.equals(permission) ||
                PermissionPool.REQUEST_INSTALL_PACKAGES.equals(permission) ||
                PermissionPool.SYSTEM_ALERT_WINDOW.equals(permission) ||
                PermissionPool.NOTIFICATION_SERVICE.equals(permission) ||
                PermissionPool.WRITE_SETTINGS.equals(permission);
    }
    ...
    static boolean isGrantedPermission(Context context, String permission) {
        ...
        // 检测安装权限
        if (PermissionPool.REQUEST_INSTALL_PACKAGES.equals(permission)) {
            return isGrantedInstallPermission(context);
        }

        // 检测悬浮窗权限
        if (PermissionPool.SYSTEM_ALERT_WINDOW.equals(permission)) {
        ...
        // 检测系统权限
        if (PermissionPool.WRITE_SETTINGS.equals(permission)) {
            return isGrantedSettingPermission(context);
        }

        // 检测 10.0 的三个新权限
        if (!isAndroid10()) {
            if (PermissionPool.ACCESS_BACKGROUND_LOCATION.equals(permission) ||
                    PermissionPool.ACCESS_MEDIA_LOCATION.equals(permission)) {
                return true;
            }

            if (PermissionPool.ACTIVITY_RECOGNITION.equals(permission)) {
                return context.checkSelfPermission(PermissionPool.BODY_SENSORS) == PackageManager.PERMISSION_GRANTED;
            }
        }

        // 检测 9.0 的一个新权限
        if (!isAndroid9()) {
            if (PermissionPool.ACCEPT_HANDOVER.equals(permission)) {
                return true;
            }
        }
        ...
        // 检测 8.0 的两个新权限
        if (!isAndroid8()) {
            if (PermissionPool.ANSWER_PHONE_CALLS.equals(permission)) {
                return true;
            }

            if (PermissionPool.READ_PHONE_NUMBERS.equals(permission)) {
                return context.checkSelfPermission(PermissionPool.READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED;
            }
        }
    ...
    static boolean isPermissionPermanentDenied(Activity activity, String permission) {
        ...
        if (!isAndroid10()) {
            ...
            if (PermissionPool.ACTIVITY_RECOGNITION.equals(permission) ) {
                return activity.checkSelfPermission(PermissionPool.BODY_SENSORS) == PackageManager.PERMISSION_DENIED &&
        ...
        if (!isAndroid8()) {
            ...
            if (PermissionPool.READ_PHONE_NUMBERS.equals(permission)) {
                return activity.checkSelfPermission(PermissionPool.READ_PHONE_STATE) == PackageManager.PERMISSION_DENIED &&
                        !activity.shouldShowRequestPermissionRationale(permission);
            }
        }
