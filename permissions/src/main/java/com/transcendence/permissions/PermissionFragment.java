...
public final class PermissionFragment extends Fragment implements Runnable {
    ...
    public void requestSpecialPermission() {
        ...
        if (PermissionUtils.containsSpecialPermission(permissions)) {
            ...
            if (permissions.contains(PermissionPool.MANAGE_EXTERNAL_STORAGE) && !PermissionUtils.isGrantedStoragePermission(activity)) {
            ...
            if (permissions.contains(PermissionPool.REQUEST_INSTALL_PACKAGES) && !PermissionUtils.isGrantedInstallPermission(activity)) {
                // 跳转到安装权限设置界面
                startActivityForResult(PermissionSettingPage.getInstallPermissionIntent(activity), getArguments().getInt(REQUEST_CODE));
                requestSpecialPermission = true;
            }

            if (permissions.contains(PermissionPool.SYSTEM_ALERT_WINDOW) && !PermissionUtils.isGrantedWindowPermission(activity)) {
                // 跳转到悬浮窗设置页面
                startActivityForResult(PermissionSettingPage.getWindowPermissionIntent(activity), getArguments().getInt(REQUEST_CODE));
                requestSpecialPermission = true;
            }

            if (permissions.contains(PermissionPool.NOTIFICATION_SERVICE) && !PermissionUtils.isGrantedNotifyPermission(activity)) {
                // 跳转到通知栏权限设置页面
                startActivityForResult(PermissionSettingPage.getNotifyPermissionIntent(activity), getArguments().getInt(REQUEST_CODE));
                requestSpecialPermission = true;
            }

            if (permissions.contains(PermissionPool.WRITE_SETTINGS) && !PermissionUtils.isGrantedSettingPermission(activity)) {
                // 跳转到系统设置权限设置页面
    ...
    public void requestDangerousPermission() {
        ...
        if (PermissionUtils.isAndroid10() && allPermissions.contains(PermissionPool.ACCESS_BACKGROUND_LOCATION)) {
            locationPermission = new ArrayList<>();
            if (allPermissions.contains(PermissionPool.ACCESS_COARSE_LOCATION) &&
                    !PermissionUtils.isGrantedPermission(activity, PermissionPool.ACCESS_COARSE_LOCATION)) {
                locationPermission.add(PermissionPool.ACCESS_COARSE_LOCATION);
            }

            if (allPermissions.contains(PermissionPool.ACCESS_FINE_LOCATION) &&
                    !PermissionUtils.isGrantedPermission(activity, PermissionPool.ACCESS_FINE_LOCATION)) {
                locationPermission.add(PermissionPool.ACCESS_FINE_LOCATION);
            }
        }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        for (int i = 0; i < permissions.length; i++) {
            ...
            if (!PermissionUtils.isAndroid10() &&
                    (PermissionPool.ACCESS_BACKGROUND_LOCATION.equals(permission) ||
                            PermissionPool.ACTIVITY_RECOGNITION.equals(permission) ||
                            PermissionPool.ACCESS_MEDIA_LOCATION.equals(permission))) {
            ...
            if (!PermissionUtils.isAndroid9() &&
                    PermissionPool.ACCEPT_HANDOVER.equals(permission)) {
                // 如果当前版本不符合最低要求，那么就重新进行权限检测
                grantResults[i] = PermissionUtils.getPermissionStatus(activity, permission);
                continue;
            }
            ...
            if (!PermissionUtils.isAndroid8() &&
                    (PermissionPool.ANSWER_PHONE_CALLS.equals(permission) ||
                            PermissionPool.READ_PHONE_NUMBERS.equals(permission))) {
                // 如果当前版本不符合最低要求，那么就重新进行权限检测
                grantResults[i] = PermissionUtils.getPermissionStatus(activity, permission);
            }
        }
        ...
        if (grantedPermission.size() == permissions.length) {
            // 代表申请的所有的权限都授予了
            Permissions.getInterceptor().grantedPermissions(activity, callBack, grantedPermission, true);
            return;
        }
        ...
        Permissions.getInterceptor().deniedPermissions(activity, callBack, deniedPermission, PermissionUtils.isPermissionPermanentDenied(activity, deniedPermission));

        // 证明还有一部分权限被成功授予，回调成功接口
        if (!grantedPermission.isEmpty()) {
            Permissions.getInterceptor().grantedPermissions(activity, callBack, grantedPermission, false);
        }
    }
