package com.transcendence.permissions;
...
final class PermissionChecker {
    ...
    static boolean checkPermissionArgument(List<String> requestPermissions, boolean debugMode) {
        ...
        if (debugMode) {
            ...
            Field[] fields = PermissionPool.class.getDeclaredFields();
    ...
    static void checkStoragePermission(Context context, List<String> requestPermissions, boolean scopedStorage) {
        ...
        if (!requestPermissions.contains(PermissionPool.MANAGE_EXTERNAL_STORAGE) &&
                !requestPermissions.contains(PermissionPool.READ_EXTERNAL_STORAGE) &&
                !requestPermissions.contains(PermissionPool.WRITE_EXTERNAL_STORAGE)) {
            return;
        }
                ...
                if (parser.getEventType() == XmlResourceParser.START_TAG) {
                    ...
                    if ("application".equals(nodeName)) {
                        ...
                        if (targetSdkVersion >= Build.VERSION_CODES.Q && !requestLegacyExternalStorage &&
                                (requestPermissions.contains(PermissionPool.MANAGE_EXTERNAL_STORAGE) || !scopedStorage)) {
    ...
    static void checkLocationPermission(List<String> requestPermissions) {
        // 判断是否包含后台定位权限
        if (!requestPermissions.contains(PermissionPool.ACCESS_BACKGROUND_LOCATION)) {
            return;
        }

        for (String permission : requestPermissions) {
            if (PermissionPool.ACCESS_FINE_LOCATION.equals(permission)
                    || PermissionPool.ACCESS_COARSE_LOCATION.equals(permission)
                    || PermissionPool.ACCESS_BACKGROUND_LOCATION.equals(permission)) {
                continue;
            }
    ...
    static void checkTargetSdkVersion(Context context, List<String> requestPermissions) {
        ...
        } else if (requestPermissions.contains(PermissionPool.ACCEPT_HANDOVER)) {
            targetSdkMinVersion = Build.VERSION_CODES.P;
        } else if (requestPermissions.contains(PermissionPool.ACCESS_BACKGROUND_LOCATION) ||
                requestPermissions.contains(PermissionPool.ACTIVITY_RECOGNITION) ||
                requestPermissions.contains(PermissionPool.ACCESS_MEDIA_LOCATION)) {
            targetSdkMinVersion = Build.VERSION_CODES.Q;
        } else if (requestPermissions.contains(PermissionPool.REQUEST_INSTALL_PACKAGES) ||
                requestPermissions.contains(PermissionPool.ANSWER_PHONE_CALLS) ||
                requestPermissions.contains(PermissionPool.READ_PHONE_NUMBERS)) {
    ...
    static void optimizeDeprecatedPermission(List<String> requestPermissions) {
        ...
        if (requestPermissions.contains(PermissionPool.MANAGE_EXTERNAL_STORAGE)) {
            ...
            if (requestPermissions.contains(PermissionPool.READ_EXTERNAL_STORAGE) ||
                    requestPermissions.contains(PermissionPool.WRITE_EXTERNAL_STORAGE)) {
            ...
            if (!PermissionUtils.isAndroid11()) {
                ...
                requestPermissions.add(PermissionPool.READ_EXTERNAL_STORAGE);
                requestPermissions.add(PermissionPool.WRITE_EXTERNAL_STORAGE);
            }
        }

        if (!PermissionUtils.isAndroid8() &&
                requestPermissions.contains(PermissionPool.READ_PHONE_NUMBERS) &&
                !requestPermissions.contains(PermissionPool.READ_PHONE_STATE)) {
            // 自动添加旧版的读取电话号码权限，因为旧版的系统不支持申请新版的权限
            requestPermissions.add(PermissionPool.READ_PHONE_STATE);
        }

        if (!PermissionUtils.isAndroid10() &&
                requestPermissions.contains(PermissionPool.ACTIVITY_RECOGNITION) &&
                !requestPermissions.contains(PermissionPool.BODY_SENSORS)) {
            // 自动添加传感器权限，因为这个权限是从 Android 10 开始才从传感器权限中剥离成独立权限
            requestPermissions.add(PermissionPool.BODY_SENSORS);
        }
    }
    ...
    static void checkPermissionManifest(Context context, List<String> requestPermissions) {
        ...
        for (String permission : requestPermissions) {
            ...
            if (minSdkVersion < Build.VERSION_CODES.R) {
                if (PermissionPool.MANAGE_EXTERNAL_STORAGE.equals(permission)) {

                    if (!manifestPermissions.contains(PermissionPool.READ_EXTERNAL_STORAGE)) {
                        // 为了保证能够在旧版的系统上正常运行，必须要在清单文件中注册此权限
                        throw new ManifestRegisterException(PermissionPool.READ_EXTERNAL_STORAGE);
                    }

                    if (!manifestPermissions.contains(PermissionPool.WRITE_EXTERNAL_STORAGE)) {
                        // 为了保证能够在旧版的系统上正常运行，必须要在清单文件中注册此权限
                        throw new ManifestRegisterException(PermissionPool.WRITE_EXTERNAL_STORAGE);
                    }
                }
            }

            if (minSdkVersion < Build.VERSION_CODES.Q) {
                if (PermissionPool.ACTIVITY_RECOGNITION.equals(permission) &&
                        !manifestPermissions.contains(PermissionPool.BODY_SENSORS)) {
                    // 为了保证能够在旧版的系统上正常运行，必须要在清单文件中注册此权限
                    throw new ManifestRegisterException(PermissionPool.BODY_SENSORS);
                }
            }

            if (minSdkVersion < Build.VERSION_CODES.O) {
                if (PermissionPool.READ_PHONE_NUMBERS.equals(permission) &&
                        !manifestPermissions.contains(PermissionPool.READ_PHONE_STATE)) {
                    // 为了保证能够在旧版的系统上正常运行，必须要在清单文件中注册此权限
                    throw new ManifestRegisterException(PermissionPool.READ_PHONE_STATE);
                }
            }

            if (PermissionPool.NOTIFICATION_SERVICE.equals(permission)) {
