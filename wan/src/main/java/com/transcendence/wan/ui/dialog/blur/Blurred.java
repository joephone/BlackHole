package com.transcendence.wan.ui.dialog.blur;
...
import com.transcendence.wan.ui.anydialog.AnydialogUtils;
...
public final class Blurred {
    ...
    private static IBlur requireBlur() {
        return AnydialogUtils.requireNonNull(sBlur, "Blurred未初始化");
    }
    ...
    public void blur(final Callback callback) {
        AnydialogUtils.requireNonNull(callback, "Callback不能为空");
        mCallback = callback;
    ...
    public void blur(final ImageView into) {
        AnydialogUtils.requireNonNull(mViewFrom, "实时高斯模糊时待模糊View不能为空");
        AnydialogUtils.requireNonNull(into, "ImageView不能为空");
        mViewInto = into;
        if (mOnPreDrawListener == null) {
