...
import com.transcendence.wan.ui.dialog.IAnim;
import com.transcendence.wan.ui.dialog.IDataBinder;
import com.transcendence.wan.ui.dialog.OnDialogCreatedListener;
import com.transcendence.wan.ui.dialog.OnDialogDismissedListener;
import com.transcendence.wan.ui.dialog.OnDialogEnterListener;
import com.transcendence.wan.ui.dialog.OnDialogExitListener;
import com.transcendence.wan.ui.dialog.OnDialogShownListener;
import com.transcendence.wan.ui.dialog.ViewHolder;
import com.transcendence.wan.ui.dialog.blur.Blurred;
...
public class AnyDialog  extends Dialog {
    ...
    private void initBackground() {
        if (mBackgroundBlurPercent > 0 || mBackgroundBlurRadius > 0) {
            Activity activity = AnydialogUtils.getActivity(mContext);
            if (activity != null) {
                ...
                Bitmap decorBitmap = AnydialogUtils.snapshot(activityDecor);
                ...
                if (blurBitmap != null) {
                    ...
                    mViewHolder.getBackground().setColorFilter(AnydialogUtils.alphaColor(mBackgroundColorInt, mBackgroundDimAmount));
                }
            }
        } else {
            if (mBackgroundColorInt == -1) {
    ...
    private void calculateInsideParamsOff() {
        mInsideParamsOff[1] = AnydialogUtils.getStatusBarHeight(mContext);
        Activity activity = AnydialogUtils.getActivity(mContext);
        if (activity != null) {
            final View activityDecor = activity.getWindow().getDecorView();
